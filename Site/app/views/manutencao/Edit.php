...
<div class="container">
    ...
    <div class="row justify-content-center mt-5">
        <div class="col-12">
            <form action="<?= URL_BASE.'manutencao/edit'?>" method="POST">
                ...
                <fieldset class="form-group border rounded p-5">
                    ...
                    <div class="form-row">
                        <div class="form-group col-md-4">
                            <label for="data_inicio date">Data de início</label>
                            <input class="form-control" type="text" id="data_inicio" name="data_inicio" value="<?=isset($manutencao['data_inicio']) ? date("d/m/Y", strtotime($manutencao['data_inicio'])) : ''?>">
                        </div>
                        <div class="form-group col-md-4">
                            <label for="data_termino">Data de término</label>
                            <input class="form-control date" type="text" id="data_termino" name="data_termino"val="<?=isset($manutencao['data_termino']) ? date("d/m/Y", strtotime($manutencao['data_termino'])) : ''?>"  placeholder="Digite a data de termino">
                        </div>
                        <div class="form-group col-md-2">
                            <label for="prazo">Prazo</label>
                            <input class="form-control" type="number" id="prazo" name="prazo" value="<?=isset($manutencao['prazo']) ? $manutencao['prazo'] : ''?>" placeholder="Prazo em dias">
                        </div>
                        <div class="form-group col-md-2">
                            <label for="status">Situação</label>
                            <select class="form-control" name="status">
                                    <option value="Iniciado" <?=isset($manutencao['status']) && $manutencao['status'] =='Iniciado' ?'selected' : ''?>>Iniciado</option>
                                    <option value="Aguardando retirada" <?=isset($manutencao['status']) && $manutencao['status'] =='Aguardando retirada' ?'selected' : ''?>>Aguardando retirada</option>
                                    <option value="Atrasado" <?=isset($manutencao['status']) && $manutencao['status'] =='Atrasado' ?'selected' : ''?>>Atrasado</option>
                                    <option value="Finalizado" <?=isset($manutencao['status']) && $manutencao['status'] =='Finalizado' ?'selected' : ''?>>Finalizado</option>
                            </select>
                        </div>
                    </div>
                </fieldset>
                <fieldset class="form-group border rounded p-5">
                    ...
                    <?php foreach($itensManutencao as $i){?>
                    <div class="form-row">
                        <div class="form-group col-md-6">
                            <label for="desc_item_1">Descrição do item</label>
                            <input class="form-control" type="text" name="itens[]"  value="<?=$i->descricao?>" placeholder="Descrição do item">
                        </div>
                        <div class="form-group col-md-4">
                            <label for="valor_item_1">Valor</label>
                            <input class="form-control" type="text"  name="valor_item[]" value="<?=$i->valor?>" placeholder="Digite o valor deste item">
                        </div>
                ...
                <fieldset class="form-group border rounded p-5">
                    ...
                    <div id="lista_pecas" class="mt-4">
                        <?php foreach($pecaManutencao as $idx=>$pm){?>
                            <hr>
                            <div class="form-row">
                                <div class="form-group col-md-6">
                                    <label>Peça adicionada</label>
                                    <input class="form-control" value="<?= $pecas[$idx]->descricao?>">
                                </div>
                                <div class="form-group col-md-3">
                                    <label>Qtd adicionada</label>
                                    <input class="form-control" value="<?= $pm->quantidade?>">
                                </div>
                                <div class="form-group col-md-3">
                                    <label>Estoque Atual</label>
                                    <input class="form-control" value="<?= $pecas[$idx]->estoque?>">
                                    <input name="peca[]" type="hidden" value="<?=$pm->id_peca?>">
                                    <input name="qtd_peca[]" type="hidden" value="<?=$pm->quantidade?>">
                                </div>
                            </div>
                        <?php }?>
                    </div>
