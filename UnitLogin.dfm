object FrmLogin: TFrmLogin
  Left = 0
  Top = 0
  ClientHeight = 211
  ClientWidth = 367
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnActivate = FormActivate
  OnClose = FormClose
  PixelsPerInch = 96
  TextHeight = 13
  object BitBtn1: TBitBtn
    Left = 282
    Top = 159
    Width = 75
    Height = 25
    Caption = '&Cancelar'
    Kind = bkCancel
    NumGlyphs = 2
    TabOrder = 2
    OnClick = BitBtn1Click
  end
  object BitBtn2: TBitBtn
    Left = 201
    Top = 159
    Width = 75
    Height = 25
    Caption = '&OK'
    Glyph.Data = {
      
      00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
    NumGlyphs = 2
    TabOrder = 1
    OnClick = BitBtn2Click
  end
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 367
    Height = 41
    Align = alTop
    TabOrder = 4
  end
  object Panel2: TPanel
    Left = 8
    Top = 48
    Width = 351
    Height = 105
    BevelInner = bvRaised
    BevelOuter = bvLowered
    TabOrder = 0
    object Label1: TLabel
      Left = 23
      Top = 8
      Width = 112
      Height = 13
      Caption = 'Login no UnimepCar'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label2: TLabel
      Left = 23
      Top = 40
      Width = 36
      Height = 13
      Caption = 'Usu'#225'rio'
    end
    object Label3: TLabel
      Left = 29
      Top = 66
      Width = 30
      Height = 13
      Caption = 'Senha'
    end
    object ValorNome: TEdit
      Left = 71
      Top = 37
      Width = 153
      Height = 21
      Hint = 'Digite seu nome de Login'
      TabOrder = 0
    end
    object ValorSenha: TEdit
      Left = 71
      Top = 63
      Width = 153
      Height = 21
      Hint = 'Digite a sua senha'
      PasswordChar = '*'
      TabOrder = 1
    end
  end
  object StatusBar1: TStatusBar
    Left = 0
    Top = 192
    Width = 367
    Height = 19
    Panels = <
      item
        Width = 50
      end>
  end
  object FDQuery1: TFDQuery
    Connection = DM.FDConnection1
    SQL.Strings = (
      
        'SELECT Login.*, Perfil.*, Funcionario.Cargo, Cargo.Descricao as ' +
        'DescricaoCargo FROM Login INNER JOIN Perfil ON Login.Perfil = Pe' +
        'rfil.Codigo INNER JOIN Funcionario ON Login.Funcionario = Funcio' +
        'nario.Codigo INNER JOIN Cargo ON Funcionario.Cargo = Cargo.Codig' +
        'o WHERE Login.Usuario = :Usu AND Login.Senha = :Sen')
    Left = 80
    Top = 232
    ParamData = <
      item
        Name = 'USU'
        ParamType = ptInput
        Value = Null
      end
      item
        Name = 'SEN'
        ParamType = ptInput
      end>
  end
end
